...
import { categories, allEffects, type CategoryId } from '@/lib/effects-data';
...
export function EffectsShowcase() {
  ...
  const filteredEffects = useMemo(() => {
    return allEffects.filter((effect) => {
      ...
      return effect.name.toLowerCase().includes(q) || effect.description.toLowerCase().includes(q) || effect.category.toLowerCase().includes(q);
    });
  ...
  const handleRandomEffect = useCallback(() => {
    ...
    if (!pool.length) { toast.info('No effects in this category'); return; }
    const pick = pool[Math.floor(Math.random() * pool.length)];
    setSearchQuery('');
    const el = document.getElementById(`effect-card-${pick.id}`);
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'center' });
      el.classList.add('ring-2', 'ring-amber-500', 'shadow-lg', 'shadow-amber-500/20');
      setTimeout(() => el.classList.remove('ring-2', 'ring-amber-500', 'shadow-lg', 'shadow-amber-500/20'), 2000);
    }
  }, [activeCategory, favorites]);

  const handleExportCSS = useCallback(async () => {
    const allCSS = allEffects.map((e) => `/* === ${e.name} === */\n${e.cssCode}\n`).join('\n');
    try {
      await navigator.clipboard.writeText(allCSS);
      toast.success(`All ${allEffects.length} effects copied!`);
    } catch {
      const blob = new Blob([allCSS], { type: 'text/css' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = 'css-effects-library.css'; a.click();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* Header */}
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-background/80 backdrop-blur-xl border-b border-border/50 shadow-sm' : 'bg-transparent'}`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-24 pb-12 sm:pb-16 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-medium mb-6">
                ...
                <span>{allEffects.length} Curated CSS Effects</span>
              </div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight">
                <span className="bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 bg-clip-text text-transparent">CSS Effects</span>{' '}
                <span className="text-foreground">Library</span>
              </h1>
              <p className="mt-4 sm:mt-6 text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                Browse, preview, favorite, and copy production-ready CSS effects. Includes a live playground and export.
              </p>
              <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-amber-500" />{categories.length - 2} Categories</span>
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />Pure CSS</span>
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-orange-500" />Copy &amp; Paste</span>
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500" />Live Playground</span>
              </div>
              <button onClick={scrollToEffects} className="mt-10 inline-flex flex-col items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
        ...
        <section id="effects-grid" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
          <div className="sticky top-14 z-40 bg-background/80 backdrop-blur-xl py-4 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 border-b border-border/30">
            <div className="flex items-center gap-3 max-w-md mx-auto mb-4">
              <div className="relative flex-1">
                ...
                <Input type="text" placeholder="Search effects..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-10 bg-muted/50 border-border/50 focus-visible:ring-amber-500/30" />
              </div>
              <FeatureBar effectCount={allEffects.length} onRandomEffect={handleRandomEffect} onExportCSS={handleExportCSS} onTogglePlayground={() => setShowPlayground(!showPlayground)} showPlayground={showPlayground} />
            </div>
            <div ref={categoryScrollRef} className="flex gap-2 overflow-x-auto pb-1 scrollbar-none -mx-1 px-1">
              {categories.map((cat) => {
                ...
                let cls = 'shrink-0 flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium transition-all duration-200 ';
                if (active) cls += 'bg-amber-500 text-black shadow-lg shadow-amber-500/25';
                ...
                return (
                  <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={cls}>
                    <span>{cat.icon}</span>
                    <span>{cat.name}</span>
                    <span className={active ? 'text-[10px] px-1.5 py-0.5 rounded-full font-semibold bg-black/15 text-black/80' : fav ? 'text-[10px] px-1.5 py-0.5 rounded-full font-semibold bg-red-500/20 text-red-400' : 'text-[10px] px-1.5 py-0.5 rounded-full font-semibold bg-muted text-muted-foreground/70'}>{cnt}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="mt-6 mb-4">
            <p className="text-sm text-muted-foreground">
              Showing <span className="font-semibold text-foreground">{filteredEffects.length}</span> {filteredEffects.length === 1 ? 'effect' : 'effects'}
          ...
          <AnimatePresence mode="wait">
            <motion.div key={`${activeCategory}-${searchQuery}-${favorites.length}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {filteredEffects.length === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {activeCategory === 'favorites' ? 'Click the heart icon on any effect to save it here' : 'Try adjusting your search or category filter'}
              </p>
      ...
      <footer className="mt-auto border-t border-border/50 bg-muted/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <p className="text-xs text-muted-foreground">{allEffects.length} pure CSS effects &middot; {categories.length - 2} categories &middot; Favorites &middot; Playground &middot; Export</p>
          </div>
        </div>
      </footer>
